package com.dma.dungeonmazemod;
...
import com.dma.dungeonmazemod.api.DungeonMazeAPI;
import com.dma.dungeonmazemod.listener.DungeonMazeBlockListener;
import com.dma.dungeonmazemod.listener.DungeonMazePlayerListener;
import com.dma.dungeonmazemod.manager.DMWorldManager;
import com.dma.dungeonmazemod.manager.PermissionsManager;
import com.dma.dungeonmazemod.Metrics.Graph;
...
public class DungeonMaze extends JavaPlugin {	
	...
	// TODO: Put the update checker into a separate class to clean up the code
	...
	private void setupMultiverse() {
		...
		if (multiversePlugin == null) {
		    log.info("[DungeonMaze] Multiverse not detected! Disabling Multiverse usage!");
		    useMultiverse = false;
		    return;
		}
		...
		if(commandLabel.equalsIgnoreCase("dungeonmaze") || commandLabel.equalsIgnoreCase("dm")) {
			...
			if(args[0].equalsIgnoreCase("createworld") ||
				...
				if(sender instanceof Player) {
					...
					if(!getPermissionsManager().hasPermission((Player) sender, "dungeonmaze.command.createworld", true)) {
				...
				if(sender instanceof Player) {
					...
					if(!getPermissionsManager().hasPermission((Player) sender, "dungeonmaze.command.teleport", true)) {
						sender.sendMessage(ChatColor.DARK_RED + "You don't have permission!");
						return true;
					}
				}
				...
				if(sender instanceof Player) {
					...
					if(!getPermissionsManager().hasPermission((Player) sender, "dungeonmaze.command.listworlds", true)) {
						sender.sendMessage(ChatColor.DARK_RED + "You don't have permission!");
						return true;
					}
				}
				...
				// Check wrong command values
				if(args.length != 1) {
					sender.sendMessage(ChatColor.DARK_RED + "Wrong command arguments!");sender.sendMessage(ChatColor.YELLOW + "Use " + ChatColor.GOLD + "/" + commandLabel + " help " + ChatColor.YELLOW + "to view help");
					return true;
				}
				...
				sender.sendMessage(ChatColor.DARK_RED + "Command not recognized!");sender.sendMessage(ChatColor.YELLOW + "Use " + ChatColor.GOLD + "/" + commandLabel + " help " + ChatColor.YELLOW + "to view help");
				return true;
			}
		}
		return false;
	...
	// Function to get a custom configuration file
	...
	// TODO: Put all this code below in a manager class to handle all the hard stuff, and to clean up the code.
